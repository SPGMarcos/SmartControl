...
const char* LOCAL_AP_SSID = "ESP_Hidroponia";
...
void setup() {
    Serial.begin(115200);
    delay(200);
    smartLog("boot hidroponia Heltec");
    u8g2.begin();
    beginRelaysOff();
    LittleFS.begin(true);
    loadSettings();
    ...
    if (MDNS.begin("smarthidroponia")) {
        ...
        smartLog("mDNS ativo smarthidroponia.local");
    } else {
    ...
    server.on("/set", [](){
        ...
        if (server.hasArg("v1") && !trava24h) { setPumpState(server.arg("v1") == "true", "dashboard"); }
        if (server.hasArg("v2") && !trava24h) { setOxygenatorState(server.arg("v2") == "true", "dashboard"); }
        saveSettings();
        publishStatus();
