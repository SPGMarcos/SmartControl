...
unsigned long mqttReconnectDelayMs = 5000UL;
unsigned long lastStatusRetryAt = 0;
...
bool pumpOutputKnown = false;
bool oxygenatorOutputKnown = false;
bool pumpOutputState = false;
bool oxygenatorOutputState = false;
bool settingsLoaded = false;
bool relayStatesLoaded = false;
String lastRequestId = "";
unsigned long lastRequestAt = 0;

const unsigned long MQTT_HEARTBEAT_INTERVAL_MS = 15000UL;
const unsigned long MQTT_RECONNECT_MIN_MS = 5000UL;
const unsigned long MQTT_RECONNECT_MAX_MS = 30000UL;
const unsigned long DUPLICATE_REQUEST_WINDOW_MS = 20000UL;
const unsigned long STATUS_RETRY_INTERVAL_MS = 5000UL;
const char* LOCAL_AP_SSID = "SmartControl";
...
void smartLog(const char* message) {
    Serial.print("[SmartControl] ");
    Serial.println(message);
}

void smartLogValue(const char* label, const String& value) {
    Serial.print("[SmartControl] ");
    Serial.print(label);
...
void saveSettings() {
    ...
    if (f) {
        ...
        f.println(v2);
        f.close();
        smartLog("configuracoes salvas no LittleFS");
    } else {
        smartLog("erro ao salvar configuracoes no LittleFS");
    }
}
...
void loadSettings() {
    if (LittleFS.exists("/config.txt")) {
        ...
        if (f) {
            ...
            if (f.available()) { v1 = f.readStringUntil('\n').toInt(); relayStatesLoaded = true; }
            if (f.available()) { v2 = f.readStringUntil('\n').toInt(); relayStatesLoaded = true; }
            ...
            f.close();
            smartLog("configuracoes carregadas do LittleFS");
        }
    } else {
        smartLog("configuracao local nao encontrada, usando padroes seguros");
    }
}

void sanitizeRuntimeSettings() {
    if (tOnMin < 1) tOnMin = 10;
    if (tOffMin < 1) tOffMin = 10;
    if (tOnMin > 1440) tOnMin = 1440;
    if (tOffMin > 1440) tOffMin = 1440;
    if (!mDeviceId.length()) mDeviceId = "hidroponia01";
    if (!mTopic.length()) mTopic = "smartcontrol/cliente/projeto/" + mDeviceId;
    if (!settingsLoaded) {
        trava24h = false;
...
void beginRelaysOff() {
    ...
    digitalWrite(RELAY_OXIGENADOR, HIGH);
    pinMode(RELAY_BOMBA, OUTPUT);
    pinMode(RELAY_OXIGENADOR, OUTPUT);
    digitalWrite(RELAY_BOMBA, HIGH);
    digitalWrite(RELAY_OXIGENADOR, HIGH);
    pumpOutputKnown = true;
    oxygenatorOutputKnown = true;
    pumpOutputState = false;
    oxygenatorOutputState = false;
    smartLog("GPIOs inicializados com reles desligados");
}

bool setPumpState(bool enabled, const char* reason) {
    if (pumpOutputKnown && pumpOutputState == enabled && v1 == enabled) return false;
    v1 = enabled;
    writeRelayPin(RELAY_BOMBA, enabled);
    pumpOutputKnown = true;
    pumpOutputState = enabled;
    Serial.print("[SmartControl] rele bomba ");
...
bool setOxygenatorState(bool enabled, const char* reason) {
    if (oxygenatorOutputKnown && oxygenatorOutputState == enabled && v2 == enabled) return false;
    v2 = enabled;
    writeRelayPin(RELAY_OXIGENADOR, enabled);
    oxygenatorOutputKnown = true;
    oxygenatorOutputState = enabled;
    Serial.print("[SmartControl] rele oxigenador ");
    Serial.print(enabled ? "ON" : "OFF");
    Serial.print(" origem=");
    Serial.println(reason);
    return true;
...
void applyConfiguredRelayState(const char* reason) {
    setPumpState(v1, reason);
    setOxygenatorState(v2, reason);
}

bool setTimers(int newOn, int newOff, const char* reason) {
    if (newOn < 1 || newOff < 1 || newOn > 1440 || newOff > 1440) {
        Serial.print("[SmartControl] timer invalido origem=");
        Serial.println(reason);
        return false;
    }

    if (tOnMin == newOn && tOffMin == newOff) return false;

    tOnMin = newOn;
    tOffMin = newOff;
    ultimoTempoBomba = millis();
    Serial.print("[SmartControl] timers atualizados origem=");
    Serial.print(reason);
    Serial.print(" on=");
    Serial.print(tOnMin);
    Serial.print(" off=");
    Serial.println(tOffMin);
    return true;
}
...
bool isStatusLikeTopic(const String& suffix) {
    return suffix == "status" || suffix == "heartbeat" || suffix == "telemetry";
}

bool publishJson(const String& suffix, JsonDocument& doc, bool retained = false) {
    if (!mqttClient.connected()) {
        if (isStatusLikeTopic(suffix)) pendingStatusPublish = true;
        return false;
    }
    ...
    bool ok = mqttClient.publish(topic.c_str(), (const uint8_t*)buffer, length, retained);
    if (!ok && isStatusLikeTopic(suffix)) pendingStatusPublish = true;

    Serial.print("[SmartControl] MQTT publish ");
    Serial.print(topic);
    Serial.print(ok ? " ok bytes=" : " falhou bytes=");
...
bool publishStatus(const char* eventType = "status") {
    ...
    doc["phase"] = trava24h ? (v1 ? "on" : "off") : "manual";
    doc["heap"] = ESP.getFreeHeap();
    doc["mqtt_state"] = mqttClient.state();
    doc["reconnect_delay_ms"] = mqttReconnectDelayMs;
    doc["uptime_ms"] = millis();

    bool ok = publishJson(eventType, doc, strcmp(eventType, "status") == 0);
    if (ok) pendingStatusPublish = false;
    return ok;
}
...
bool applyAutomaticMode(bool enabled) {
    bool changed = false;
    if (trava24h != enabled) {
        trava24h = enabled;
        changed = true;
        Serial.print("[SmartControl] modo ");
        Serial.println(enabled ? "automatico" : "manual");
    }

    ultimoTempoBomba = millis();
    if (enabled) {
        changed |= setOxygenatorState(true, "automatico");
        changed |= setPumpState(true, "automatico");
    }
...
bool applyRemoteConfig(JsonObject data, String requestId, String command) {
    ...
    if (data.containsKey("enabled") || data.containsKey("automatic") || data.containsKey("t24")) {
        recognized = true;
        bool enabled = data.containsKey("enabled") ? data["enabled"].as<bool>() :
                       data.containsKey("automatic") ? data["automatic"].as<bool>() :
                       data["t24"].as<bool>();
        changed |= applyAutomaticMode(enabled);
    }
    ...
    if (data.containsKey("tOn") || data.containsKey("tOff")) {
        ...
        changed |= setTimers(newOn, newOff, command.c_str());
    }
    ...
    publishAck(requestId, command, true, changed ? "ok" : "unchanged");
    publishStatus();
    return true;
}

bool isDuplicateRequest(const String& requestId) {
    return requestId.length() &&
           requestId == lastRequestId &&
           millis() - lastRequestAt < DUPLICATE_REQUEST_WINDOW_MS;
}
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
    ...
    Serial.print("[SmartControl] MQTT comando recebido topic=");
    Serial.print(topic);
    ...
    if (isDuplicateRequest(requestId)) {
        smartLogValue("MQTT comando duplicado ignorado request=", requestId);
        publishAck(requestId, command, true, "duplicate_ignored");
        return;
    }
    ...
    if (command == "set_auto") {
        bool enabled = data.containsKey("enabled") ? data["enabled"].as<bool>() :
                       data.containsKey("automatic") ? data["automatic"].as<bool>() :
                       data["t24"].as<bool>();
        bool changed = applyAutomaticMode(enabled);
        if (changed) saveSettings();
    ...
    if (command == "set_relay") {
        ...
        bool value = data.containsKey("value") ? data["value"].as<bool>() :
                     data.containsKey("enabled") ? data["enabled"].as<bool>() :
                     data["state"].as<bool>();

        if (relay == "pump" || relay == "v1" || relay == "bomba") {
            setPumpState(value, "mqtt");
        } else if (relay == "oxygenator" || relay == "v2" || relay == "oxigenador") {
            setOxygenatorState(value, "mqtt");
        } else {
...
void reconnectMQTT() {
    ...
    if (millis() - lastMqttReconnectAttempt < mqttReconnectDelayMs) return;
    ...
    if (mqttClient.connect(clientId.c_str(), user, pass, willTopic.c_str(), 1, true, "{\"status\":\"offline\"}")) {
        ...
        publishStatus();
        smartLog("MQTT conectado e inscrito");
    } else {
        Serial.print("[SmartControl] MQTT falha state=");
        Serial.println(mqttClient.state());
        mqttReconnectDelayMs = min(mqttReconnectDelayMs * 2UL, MQTT_RECONNECT_MAX_MS);
    }
}
...
<body>
    ...
    <script>
        function update(k,v){ fetch(`/set?${k}=${v}`).then(load); }
        function load(){
...
void setup() {
    Serial.begin(115200);
    delay(200);
    smartLog("boot hidroponia ESP32");
    beginRelaysOff();
    LittleFS.begin(true);
    loadSettings();
    sanitizeRuntimeSettings();
    applyConfiguredRelayState("boot_config");

    WiFi.setSleep(false);
    WiFiManager wm;
    wm.setConnectTimeout(15);
    wm.setConfigPortalTimeout(60);
    bool wifiConnected = wm.autoConnect(LOCAL_AP_SSID);
    if (wifiConnected) {
        smartLogValue("WiFi conectado IP=", WiFi.localIP().toString());
    } else {
        smartLog("WiFi sem STA, seguindo offline/local");
    }
    WiFi.mode(WIFI_AP_STA);
    if (WiFi.softAP(LOCAL_AP_SSID)) {
        smartLogValue("AP local ativo IP=", WiFi.softAPIP().toString());
    } else {
        smartLog("falha ao iniciar AP local");
    }

    smartLogValue("IP STA=", WiFi.localIP().toString());

    if (MDNS.begin("smartcontrol")) {
        MDNS.addService("http", "tcp", 80);
        smartLog("mDNS ativo smartcontrol.local");
    } else {
        smartLog("mDNS falhou");
    }

    ArduinoOTA.setHostname("SmartControl");
    ArduinoOTA.onStart([]() {
        smartLog("OTA iniciado");
    });
    ArduinoOTA.begin();
    ...
    server.on("/set", [](){
        ...
        if (server.hasArg("tOn") || server.hasArg("tOff")) {
            int nextOn = server.hasArg("tOn") ? server.arg("tOn").toInt() : tOnMin;
            int nextOff = server.hasArg("tOff") ? server.arg("tOff").toInt() : tOffMin;
            if (!setTimers(nextOn, nextOff, "dashboard")) {
                if (nextOn < 1 || nextOff < 1 || nextOn > 1440 || nextOff > 1440) {
                    server.send(400, "text/plain", "TIMER_INVALIDO");
                    return;
                }
            }
        }
        if (!trava24h) {
             if (server.hasArg("v1")) { setPumpState(server.arg("v1") == "true", "dashboard"); }
             if (server.hasArg("v2")) { setOxygenatorState(server.arg("v2") == "true", "dashboard"); }
        }
        saveSettings();
        publishStatus();
    ...
    server.begin();
    smartLog("dashboard local iniciado");
}
...
void smartControlMqttLoop() {
    ...
    if (mqttClient.connected() && pendingStatusPublish && millis() - lastStatusRetryAt > STATUS_RETRY_INTERVAL_MS) {
        lastStatusRetryAt = millis();
        publishStatus();
    }
...
void loop() {
    ...
    if (trava24h) {
        ...
        if (millis() - ultimoTempoBomba >= intervalo) {
            ultimoTempoBomba = millis();
            setPumpState(!v1, "timer_automatico");
            setOxygenatorState(true, "timer_automatico");
            publishStatus();
        }
    }
}
